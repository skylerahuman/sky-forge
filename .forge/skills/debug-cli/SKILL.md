---
name: debug-cli
description: Use when users need to debug, modify, or extend the SkyBolt application's CLI commands, argument parsing, or CLI behavior. This includes adding new commands, fixing CLI bugs, updating command options, or troubleshooting CLI-related issues.
---

# CLI Debug Skill

This skill provides a systematic workflow for debugging and verifying changes to the sky CLI application.

## Core Principles

1. **Always get latest docs first**: Run `--help` to see current commands and options
2. **Use `-p` for testing**: Test sky by giving it tasks with the `-p` flag
...
./target/debug/sky [COMMAND] --help

# Subcommand help
./target/debug/sky [COMMAND] [SUBCOMMAND] --help
```

### 3. Test with `-p` Flag

Use the `-p` flag to give sky a task to complete without interactive mode:

```bash
# Test with a simple prompt
./target/debug/sky -p "create a hello world rust program"

# Test with specific functionality
./target/debug/sky -p "read the README.md file and summarize it"

# Test with complex tasks
./target/debug/sky -p "analyze the code structure and suggest improvements"
```
...
./target/debug/sky conversation dump <conversation-id>

# Export as HTML for human-readable format (creates: YYYY-MM-DD_HH-MM-SS-dump.html)
./target/debug/sky conversation dump --html <conversation-id>

# Use dumped JSON to reproduce issues
./target/debug/sky --conversation 2025-11-23_12-28-52-dump.json
```
...
./target/debug/sky conversation clone <source-conversation-id>

# This creates a new conversation ID - use that for testing
./target/debug/sky --conversation-id <new-cloned-id>
...
cargo build
./target/debug/sky --help  # Verify new command appears
./target/debug/sky new-command --help  # Check command docs
./target/debug/sky -p "test the new feature"
```
...
./target/debug/sky conversation dump <bug-conversation-id>

# 2. Clone it for testing (preserves original)
./target/debug/sky conversation clone <bug-conversation-id>

# 3. Reproduce with the cloned conversation
./target/debug/sky --conversation-id <cloned-id> -p "reproduce the issue"

# 4. After fix, verify with new clone
./target/debug/sky conversation clone <bug-conversation-id>
./target/debug/sky --conversation-id <new-clone-id> -p "verify fix"
```
...
./target/debug/sky command

# Test with invalid input
./target/debug/sky -p "invalid task with special chars: <>|&"

# Test with boundary values
./target/debug/sky -p "create a file with a very long name..."
```
...
./target/debug/sky conversation dump <id>
...
./target/debug/sky conversation dump --html <id>

# 4. Test modified prompts
./target/debug/sky -p "your optimized prompt here"
```
...
1. **Build**: `cargo build`
2. **Docs**: `./target/debug/sky --help` (verify documentation)
3. **Test**: `./target/debug/sky -p "relevant task"`
4. **Verify**: Check output matches expectations

### Debugging a Bug Report

1. **Clone**: `./target/debug/sky conversation clone <source-id>`
2. **Build**: `cargo build` (with potential fixes)
3. **Test**: `./target/debug/sky --conversation-id <cloned-id> -p "reproduce"`
...
cargo build
./target/debug/sky --help  # Always check docs first
./target/debug/sky -p "your test task"
...
# Clone and test bug
./target/debug/sky conversation clone <source-id>
./target/debug/sky --conversation-id <cloned-id> -p "reproduce bug"

# Debug prompts with jq (use actual filename)
cat 2025-11-23_12-28-52-dump.json | jq '.messages[] | {role, content}'

# Test with verbose output
./target/debug/sky --verbose -p "test task"
```
...
- **Use relative paths**: Binary is at `./target/debug/sky` from project root
